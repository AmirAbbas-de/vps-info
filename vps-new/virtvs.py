...
class virtapi:
        ...
        try:
            if post:
                ...
                response = requests.get(
                    url, cookies=cookies, headers=headers, verify=False, timeout=(10, 60)
                )
    ...
    def config_slave(self, post=None):
        ...
        if post.get("serid"):
            path += "&changeserid=" + str(post["serid"])
        result = self.call(path, {}, post)
    ...
    def server_stats(self, serid):
        result = self.call(f"index.php?act=server_stats&changeserid={serid}")
        if not isinstance(result, dict):
            return {}
        return result.get("server_stats", result)
