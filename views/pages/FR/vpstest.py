...
import ipaddress
import re
import jdatetime  # Added for Hejri Shamsi conversion
...
def main():
    ...
    # Get current Shamsi time for the filename
    now_shamsi = jdatetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    ...
    # Save IP Pools with Timestamp
    pool_filename = f'/var/www/html/views/pages/FR/ippools_{now_shamsi}.json'
    ...
    for vid in all_vps_ids:
        ...
        shamsi_time = unix_to_shamsi(vinfo.get('time'))
        shamsi_edittime = unix_to_shamsi(vinfo.get('edittime'))
        ...
        final_vps_data[str(vid)] = {
            ...
            "creation_date_shamsi": shamsi_time,      # New Field
            "edit_date_shamsi": shamsi_edittime        # New Field
        }

    # Save VPS Data with Timestamp
    vps_filename = f'/var/www/html/views/pages/FR/vps_{now_shamsi}.json'
